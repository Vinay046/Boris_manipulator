...
def control_callback(req):
    ...
    if not control_message:
        ...
        # response.distance_y = 2
        # response.distance_z = 3
        response.distance_x = float(controlsSplit[1])
        response.distance_y = float(controlsSplit[3])
        response.distance_z = float(controlsSplit[5])
...
localIP     = '192.168.78.66'  #"192.168.78.57" add the right IP address later
